...
public class WalletAdapter implements WalletPort {
    ...
    public void debitSubscriptionPlan(UUID userId, TypePlan plan) {
        try {
            log.info("Debiting wallet for user {} - plan: {}, amount: {}", userId, plan, plan.getPrice());
            ...
            log.error("Insufficient balance for user {} - plan: {}", userId, plan, e);
            throw new WalletTransactionException("Saldo insuficiente na carteira para realizar a assinatura do plano " + plan.getDescription());
